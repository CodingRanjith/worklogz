...
import { FiPlus, FiEdit2, FiTrash2, FiSearch, FiX, FiTrendingUp, FiTrendingDown, FiDollarSign, FiFileText, FiEye, FiBarChart2 } from 'react-icons/fi';
...
const IncomeExpense = () => {
  ...
  const fetchRecords = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get(API_ENDPOINTS.getAllIncomeExpense, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      if (response.data.success) {
        ...
        const uniqueSourceTypes = [...new Set(response.data.records.map(r => r.sourceType))];
        const uniqueMethods = [...new Set(response.data.records.map(r => r.transactionMethod))];
  ...
  const fetchSummary = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(API_ENDPOINTS.getIncomeExpenseSummary, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleOpenDialog = (record = null) => {
    if (record) {
      ...
      setFormData({
        ...
        userId: record.user._id || currentUser?._id || ''
      });
    } else {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'userId') {
      ...
      if (selectedUser) {
        console.log('User changed to:', selectedUser.name);
      }
    }
  };
  ...
  const addTransactionMethod = () => {
    if (newTransactionMethod.trim() && !transactionMethodOptions.includes(newTransactionMethod.trim())) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        userId: formData.userId || currentUser?._id || undefined
      };

      let response;
      if (editingRecord) {
        response = await axios.put(
          API_ENDPOINTS.updateIncomeExpense(editingRecord._id),
          payload,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      } else {
        response = await axios.post(
          API_ENDPOINTS.createIncomeExpense,
          payload,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      }
      ...
      if (response.data.success) {
        Swal.fire('Success', editingRecord ? 'Record updated successfully' : 'Record added successfully', 'success');
        handleCloseDialog();
        fetchRecords();
        fetchSummary();
      }
      ...
      const errorMessage = error.response?.data?.error || error.response?.data?.message || error.message || 'Failed to save record';
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await axios.delete(
          API_ENDPOINTS.deleteIncomeExpense(id),
          { headers: { Authorization: `Bearer ${token}` } }
        );
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl p-6 mb-6 shadow-lg">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">Income & Expense Management</h1>
            <p className="text-indigo-100 text-sm">Track and manage all your financial transactions</p>
          </div>
      ...
      <div className="flex flex-wrap lg:flex-nowrap gap-6 mb-6">
        ...
        <div className="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-6 text-white shadow-lg flex-1 min-w-[200px]">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-white">{formatCurrency(summary.totalExpense)}</p>
            </div>
            <FiTrendingDown className="w-12 h-12 text-white opacity-80" />
          </div>
        </div>

        <div className={`bg-gradient-to-br ${summary.balance >= 0 ? 'from-blue-500 to-blue-600' : 'from-orange-500 to-orange-600'} rounded-xl p-6 text-white shadow-lg flex-1 min-w-[200px]`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`${summary.balance >= 0 ? 'text-blue-100' : 'text-orange-100'} text-sm font-medium mb-1`}>Balance</p>
        ...
        <div className={`bg-gradient-to-br ${summary.balance >= 0 ? 'from-emerald-500 to-emerald-600' : 'from-rose-500 to-rose-600'} rounded-xl p-6 text-white shadow-lg flex-1 min-w-[200px]`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`${summary.balance >= 0 ? 'text-emerald-100' : 'text-rose-100'} text-sm font-medium mb-1`}>Profit</p>
              <p className="text-3xl font-bold text-white">{formatCurrency(summary.balance)}</p>
              <p className={`${summary.balance >= 0 ? 'text-emerald-100' : 'text-rose-100'} text-xs mt-1 opacity-90`}>
                {summary.totalIncome > 0 ? `${((summary.balance / summary.totalIncome) * 100).toFixed(1)}% margin` : '0% margin'}
              </p>
            </div>
            <FiDollarSign className="w-12 h-12 text-white opacity-80" />
          </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Source Type</th>
                  ...
                  return (
                    <tr key={record._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${sourceColor.bg} ${sourceColor.text}`}>
                          {record.sourceType}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
      ...
      {openDialog && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 transition-opacity bg-gray-900 bg-opacity-50" onClick={handleCloseDialog}></div>
                ...
                <div className="px-6 py-6 space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      {formData.sourceType && (
                        <div className="mt-2">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${getSourceTypeColor(formData.sourceType).bg} ${getSourceTypeColor(formData.sourceType).text}`}>
                            {formData.sourceType}
                          </span>
                        </div>
                      )}
                      <div className="mt-2 flex gap-2">
                        <input
                          ...
                          onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addSourceType())}
                    ...
                    <div className="relative" ref={givenByRef}>
                      ...
                      {givenByOpen && (givenBySearch || filteredUsers.length > 0) && (
                        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-80 overflow-y-auto">
                          ...
                          {givenBySearch && !filteredUsers.find(u => u.name.toLowerCase() === givenBySearch.toLowerCase()) && (
                            <div
                              onClick={handleGivenByCustom}
                              className="px-4 py-3 hover:bg-indigo-50 cursor-pointer bg-indigo-50/50"
                            >
                              <p className="text-sm font-medium text-indigo-900">Use "{givenBySearch}"</p>
                    ...
                    {/* Transaction Method */}
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Transaction Method *</label>
                      <select
                      ...
                      {formData.transactionMethod && (
                        <div className="mt-2">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${getTransactionMethodColor(formData.transactionMethod).bg} ${getTransactionMethodColor(formData.transactionMethod).text}`}>
                            {formData.transactionMethod}
                          </span>
                        </div>
                      )}
                      <div className="mt-2 flex gap-2">
                        <input
                          ...
                          onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addTransactionMethod())}
                          className="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                        />
                        <button
                          type="button"
                    ...
                    <div>
                      ...
                      >
                        {currentUser && (
                          <option value={currentUser._id}>{currentUser.name} (Current User)</option>
                        )}
      ...
      {viewingRecord && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 transition-opacity bg-gray-900 bg-opacity-50" onClick={() => setViewingRecord(null)}></div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm font-semibold text-gray-900">{formatDate(viewingRecord.date)}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-500 mb-1">Transaction Type</label>
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${
                        viewingRecord.transactionType === 'Income' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                      }`}>
                  ...
                  <div>
                    ...
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${getSourceTypeColor(viewingRecord.sourceType).bg} ${getSourceTypeColor(viewingRecord.sourceType).text}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className={`text-sm font-semibold ${viewingRecord.credit > 0 ? 'text-green-600' : 'text-gray-400'}`}>
                    ...
                    <div>
                      ...
                      <p className={`text-sm font-semibold ${viewingRecord.debit > 0 ? 'text-red-600' : 'text-gray-400'}`}>
                  ...
                  {/* Transaction Method */}
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">Transaction Method</label>
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${getTransactionMethodColor(viewingRecord.transactionMethod).bg} ${getTransactionMethodColor(viewingRecord.transactionMethod).text}`}>
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{viewingRecord.user?.name || '-'}</p>
                  ...
                  {viewingRecord.createdBy && (
                    <div>
                      ...
                      <p className="text-sm font-semibold text-gray-900">{viewingRecord.createdBy?.name || '-'}</p>
